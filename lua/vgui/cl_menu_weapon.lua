...
concommand.Add("tr_presets_open_menu", function(ply, cmd, name_items)
    local name_main_folder = name_items[1]
    -- print(name_main_folder)
    local function LoadPresetTR(name_preset)
        ...
        confirmButton.DoClick = function()
            local files, _ = file.Find(name_main_folder.."/presets/" .. name_preset .."/*.txt", "DATA")

            -- Пройдитесь по каждому файлу
            for _, filename in ipairs(files) do
                -- Прочитайте содержимое файла
                local content = file.Read(name_main_folder.."/presets/".. name_preset .. "/" .. filename, "DATA")

                -- Если содержимое существует, записывайте его в новую папку
                file.Write(name_main_folder.."/" .. filename, content)
            end
            frame:Close()
        end
    ...
    local function SavePresetTR(name_preset)
        local files, _ = file.Find(name_main_folder.."/*.txt", "DATA")

        -- Пройдитесь по каждому файлу
        for _, filename in ipairs(files) do
            -- Прочитайте содержимое файла
            local content = file.Read(name_main_folder.."/".. filename, "DATA")

            -- Если содержимое существует, записывайте его в новую папку
            if content then
                file.Write(name_main_folder.."/presets/".. name_preset .. "/" .. filename, content)
            end
        end
    end
    ...
    local function DeletePresetTR(name_preset)
        ...
        for _, filename in ipairs(files) do
            file.Delete(name_main_folder.."/presets/".. name_preset .. "/" .. filename)
            file.Delete(name_main_folder.."/presets/".. name_preset)
        end
    end

    local function CreateFoldersTR(name_preset)
        if not file.Exists(name_main_folder.."/presets/".. name_preset, "DATA") then
            file.CreateDir(name_main_folder.."/presets/".. name_preset)
        end
    end
    ...
    local function OpenPresetsMenuTR()
        ...
        presetsButton_in_save.DoClick = function()
            ...
            local name_map_from_button = "any"
            local button_map = vgui.Create("DButton", frame)
            button_map:SetSize(140, 30)
            button_map:SetPos(10, 200)
            button_map:SetText("Set a special map")
            button_map.DoClick = function()
                button_map:SetText("Set a "..currentMap_TR)
                name_map_from_button = currentMap_TR
            end
            ...
            button.DoClick = function()
                local namePresetTR = textentry:GetValue() -- Получаем введенное имя
                button:SetText("Saved!")
                CreateFoldersTR(namePresetTR.." _-_ "..name_map_from_button)
                SavePresetTR(namePresetTR.." _-_ "..name_map_from_button)
                presetsList:AddLine(namePresetTR.." _-_ "..name_map_from_button)
                frame:Close()
            end
        end
        ...
        local _, folders_presets = file.Find(name_main_folder.."/presets/*", "DATA")
...
concommand.Add("tr_weapon_menu", function(ply, cmd, args)
    ...
    presetsButton.DoClick = function()
        RunConsoleCommand( "tr_presets_open_menu", "total_weapon_replacer" )
    end
